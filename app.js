...
app.post('/',function(req,res){
    ...
    https.get(url,options,function(response){
        ...
        response.on('data',function(data){
            ...
            const active = GSTdata.data.sts
            const einvoice = GSTdata.data.einvoiceStatus
            const lastupd = GSTdata.data.lstupdt
            console.log(GSTdata)
            ...
            if(response.statusCode ===200 & GSTdata.error===false){
                res.render("list",{name:name,status:status,GSTO:GST_ofc,type:type,date:date,ao:ao,active:active,einvoice:einvoice,lastupd:lastupd});
